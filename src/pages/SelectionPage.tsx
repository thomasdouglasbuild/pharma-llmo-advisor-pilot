...
const SelectionPage = () => {
  ...
  if (companiesQuery.isError || productsQuery.isError) {
    const errorMessage = (companiesQuery.error?.message || productsQuery.error?.message) as string;
  ...
  if (companiesQuery.isLoading || productsQuery.isLoading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardHeader>
  ...
  const isEmpty = companyOptions.length === 0 && productOptions.length === 0;
